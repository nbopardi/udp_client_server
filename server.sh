...
helpFunction()
{
	...
	echo "\t-b_port Port number of the HTTP backend server (default: 80)"
	echo "\t-port Port number of the server (default: 40000)"
	echo "\t-w_time Max number of seconds the connection will wait on a full send queue to free up to send a packet (default: 5)"
	echo "\t-r_time Max number of seconds the server will wait to receive a request from client before closing (default: 10)"
	echo "\t-n_jobs Max number of requests made to the HTTP backend occurring at the same time (default: 15000)"
	echo "\t-ec_time Amount of seconds to wait for the HTTP backend's first response headers after fully writing the request headers (default: 4)"
	echo "\t-rh_time Amount of seconds to wait for the HTTP backend's response headers after fully writing the request and body (default: 10)"
	echo "\t-ic_time Max amount of seconds an idle (keep-alive) connection will remain idle before closing itself (default: 10)"
	echo "\t-iconn_host Max idle (keep-alive) connections to keep per-host (default: 10000)"
	echo "\t-buffer The max buffer size of the channel used to store received packets that are reflected to client (default: 1000000)"
